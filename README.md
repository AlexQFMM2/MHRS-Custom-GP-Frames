...
- 当前模式：独立功能开关，不修改 GP 帧
- 默认行为：关闭
- 触发：本地玩家圆月自然结束、收刀、出圈或被原版回收后，下一帧自动重开
...
开启后，脚本会捕获本地玩家通过原版 `createSpacingShell()` 创建的圆月本体 `LongSwordShell010`。当这个圆月自然结束、收刀、出圈或被原版回收时，脚本只排队一次，在下一帧调用原版创建入口重开圆月。

这版不再修改 `_moveParam._lifeTime`，也不写调试日志；相比早期每 12 帧刷新一次圆月的参考做法，这里只在消失事件后重开，避免持续刷新带来的额外开销。

## 圆月后续修改记录

当前先记录 BGM 调研方向；持续时间、范围和视觉圈修改已从运行时代码撤回。
...
- 结束检测：优先查 `LongSwordShellManager` 是否仍有玩家的 `LongSwordShell010`；如果读不到稳定接口，再用圆月消失事件或配置秒数兜底停止
