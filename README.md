...
- 功能：`自由态自动见切`
- 当前模式：独立功能开关，不修改 GP 帧
- 触发条件：武器已拔出，并且当前节点是 `atk.atk_wait` 或其子节点
- 自动见切目标节点：
  - `atk.atk_128.atk_138`
...
- 玩家武器已拔出
- 当前动作树节点是 `atk.atk_wait` 或 `atk.atk_wait.*`

满足条件后，脚本会在受击时尝试把行为树推进到参考 `Endless_Lance` 里用于接精准防御的节点。攻击动作中受击不会再自动 GP。
...
满足条件并受到敌对攻击时，脚本会直接把行为树推进到：

- `atk.atk_128.atk_138`

这项功能当前只负责“自动进入见切动作并拦截本次伤害”，不会额外补练气等级、成功伤害链或其他奖励。
