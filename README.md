...
也就是说，当前这两个示例都不是按“持续时长换算”来写，而是直接把 `_EndFrame` 改成滑条值。
...
- 运行时查找
  负责找到当前玩家、当前动作树、当前武器类型、以及指定的 `ActionIndex`。
...
现在这版正处于第 2 步：已经有完整 UI 框架，并用长枪做了第一个可用示例。
